...
def radar_factory(num_vars, frame="polygon", special_offsets=None,
                  default_offset=0.0, label_pad=18):
    ...
    nudge of individual labels and are OFF by default (``default_offset=0.0``,
    ...
    class RadarAxes(PolarAxes):
        ...
        def set_varlabels(self, labels):
            ...
            # Push every label radially off the polygon edge. This is what keeps
            # the model names from overlapping the radar, for any num_vars.
            ...
            for idx, (angle, label) in enumerate(zip(angles_deg, self.get_xticklabels())):
                ...
                # Optional fine nudge: inactive while default_offset/special_offsets
                # are empty (the radial label_pad already handles the spacing).
...
def plot_radar(results, experiment1, run, metric1,
    ...
    N = len(models)
    theta = radar_factory(N, frame="polygon", label_pad=label_pad)
...
def plot_radar_3way(results, experiments, run, metric,
                    ...
                    label_pad=18, ax=None, panel_label=None, include_extra=False):
