...
# Default display order of the models (KGE decoders).
# To add models: extend this list, or let them be detected automatically
# (they will be appended at the end of the radar).
DEFAULT_MODEL_ORDER = ["ANALOGY", "ComplEx", "HolE", "RESCAL", "DistMult", "TransE", "TransD"]
...
# ---------------------------------------------------------------------------
def radar_factory(num_vars, frame="polygon", special_offsets=None, default_offset=-0.055):
    """Register and return the 'radar' projection for ``num_vars`` axes.

    ``special_offsets`` is a dict {label_index: vertical_offset} used to nudge
    specific labels (useful when the polygon overlaps a label). By default it
    reproduces the historical tuning for a 7-model layout.
    """
    theta = np.linspace(0, 2 * np.pi, num_vars, endpoint=False)

    if special_offsets is None:
        # Original tuning, designed for 7 labels (the bottom label, index 5,
        # was pushed slightly further). Only has an effect when num_vars == 7.
        special_offsets = {5: -0.075} if num_vars == 7 else {}
    ...
    class RadarAxes(PolarAxes):
        ...
        def set_varlabels(self, labels):
            ...
            for idx, (angle, label) in enumerate(zip(angles_deg, self.get_xticklabels())):
                ...
                # Leave the labels exactly at the top / bottom (0 / 180 / 360 deg) untouched.
...
def plot_radar(results, experiment1, run, metric1,
    ...
    ax.set_rgrids(rgrid, angle=np.degrees(theta[N - 1]), ha="center",
    ...
    if labels:
        ax.legend(lines, labels, loc="lower center", bbox_to_anchor=(0.5, -0.15),
